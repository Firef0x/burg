		...
		const grub_partition_t partition, void *closure)
{
  ...
  if (c->p_offset == part_start)
    {
      if (partition->parent)
	{
	  c->dos_part = partition->parent->number;
	  c->bsd_part = partition->number;
	}
      else
	{
	  c->dos_part = partition->number;
	  c->bsd_part = -1;
	}
...
# else /* defined(__NetBSD__) */
    ...
    c.p_offset = label.d_partitions[index].p_offset;
# endif /* !defined(__NetBSD__) */

    grub_util_info ("%s starts from %llu", os_dev, c.p_offset);

    if (c.p_offset == 0 && device_is_wholedisk (os_dev))
      return name;
    ...
    grub_partition_iterate (disk, find_partition, &c);

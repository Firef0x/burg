...
static boolean detect_bios_type(vbios_map * map, boolean modeline, int entry_size) {
    int i;
    short int r1, r2;
    (void) modeline;
    ...
    for (i=0; i < (int) map->mode_table_size; i++) {
...
static vbios_map * open_vbios(chipset_type forced_chipset) {
    ...
    map->bios_ptr=(address)VBIOS_START;
            ...
            case BT_1:
                {
                    ...
                    res->x2 = (htotal? (int) (((htotal-x) >> 8) & 0x0f) : (res->x2 & 0x0f)) | ((x >> 4) & 0xf0);
                    res->x1 = (x & 0xff);
                    
                    res->y2 = (vtotal? (int) (((vtotal-y) >> 8) & 0x0f) : (res->y2 & 0x0f)) | ((y >> 4) & 0xf0);
                    res->y1 = (y & 0xff);
		    if (htotal)
...
static int parse_args(int argc, char *argv[], chipset_type *forced_chipset, cardinal *list, cardinal *mode, cardinal *x, cardinal *y, cardinal *bp, cardinal *raw, cardinal *htotal, cardinal *vtotal) {
  int index = 0;
...
{
    vbios_map * map;
    cardinal list, mode, x, y, bp = 0, raw, htotal, vtotal;
...
static grub_err_t
grub_cmd_915resolution (struct grub_command *cmd __attribute__ ((unused)),
			int argc, char *argv[])
{

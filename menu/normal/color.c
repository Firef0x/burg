...
grub_parse_color_name_pair (grub_uint8_t *ret, const char *name)
{
  ...
  if (bg_name == NULL)
    {
      grub_printf (N_("Warning: syntax error (missing slash) in `%s'\n"), fg_name);
      grub_wait_after_message ();
  ...
  *(bg_name++) = '\0';
  ...
  if (parse_color_name (&fg, fg_name) == -1)
    {
      grub_printf (N_("Warning: invalid foreground color `%s'\n"), fg_name);
      grub_wait_after_message ();
      goto free_and_return;
    }
  if (parse_color_name (&bg, bg_name) == -1)
    {
      grub_printf (N_("Warning: invalid background color `%s'\n"), bg_name);
      grub_wait_after_message ();
      goto free_and_return;
    }
